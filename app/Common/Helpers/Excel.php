...
class Excel {
    ...
    public static function output($filename = 'excel', $format = 'Xlsx') {
        ...
        $contentType = ($inTypeList) ? self::$_writerTypeInfo[$format]['contentType'] : 'application/octet-stream';
    ...
    public static function save($filename = 'excel', $format = 'Xlsx') {
        ...
        $filepath = (stripos($filename, $extension) === (strlen($filename) - strlen($extension))) ? $filename : "{$filename}{$extension}";
    ...
    public static function getRow($toString = true, $options = [], callable $callback = null) {
        ...
        for ($col = $startColumn + 1; $col <= $columns; ++$col) {
            ...
            if ($options['timestamp'] && \PhpOffice\PhpSpreadsheet\Shared\Date::isDateTime($cell)) {
                ...
                $value = ($options['timestampFormat']) ? date($options['timestampFormat'], $value) : $value;
            }
    ...
    public static function getRows($toString = true, Array $options = [], callable $callback = null) {
        ...
        $rowOffset = ($options['rowOffset'] && $options['rowOffset'] <= $highestRow) ? $options['rowOffset'] : 0;
        $rows = ($options['rows'] && ($rowOffset + $options['rows']) < $highestRow) ? $options['rows'] : $highestRow - $rowOffset;
    ...
    public static function setAutoSize($colAlphaStart = NULL, $colAlphaEnd = NULL, $value = true) {
        ...
        $colEnd = ($colAlphaEnd) ? self::alpha2num($colAlphaEnd) : self::alpha2num($sheetObj->getHighestColumn());
