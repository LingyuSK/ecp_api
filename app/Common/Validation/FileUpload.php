...
class FileUpload {
    ...
    private $path = "./Data/upload";          //上传文件保存的路径
    ...
    function upload($fileField) {
        ...
        $name = $this->getFileName($_FILES[$fileField]['name']);
     ...
     */
    public function getFileName($name = '1.jpg') {
        $ext = explode('.', $name);
        $arr = time() . '.' . $ext[1];
        return $arr;
    }
    ...
    private function proRandName() {
        ...
        return $fileName . '.' . strtolower($this->fileType);
    }
    ...
    private function copyFile() {
        if (!$this->errorNum) {
            ...
            if (@move_uploaded_file($this->tmpFileName, $path)) {
                $this->imagePngSizeAdd($path);

                return $this->imgHandle($path);
            } else {
    ...
    public function imgHandle($path) {
    ...
    public function getPercent($path) {
        list($width, $height, $type) = getimagesize($path);
        $percent = $height / 750;
        return $percent;
    }
