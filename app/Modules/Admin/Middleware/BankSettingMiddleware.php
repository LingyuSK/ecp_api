    ...
    public function handle(Request $request, Closure $next) {
        ...
        switch (strtolower($action)) {
            case 'add':
                ...
                $count = BankSetting::where('name', trim($request->name))
                        ->count();
                if (!empty($count)) {
                    check(false, '行名行号已存在');
                }
                $countN = BankSetting::where('number', trim($request->number))
                        ->count();
                if (!empty($countN)) {
                ...
                break;
            case 'edited':
                $query = BankSetting::where('id', $request->id);
                $object = $query->first();
                if (empty(trim($request->name))) {
                    check(false, '行名行号名称不能为空');
                }
                ...
                $count = BankSetting::whereNot('id', $request->id)
