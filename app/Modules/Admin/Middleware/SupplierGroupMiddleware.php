    ...
    public function handle(Request $request, Closure $next) {
        ...
        switch (strtolower($action)) {
            case 'add':
                ...
                $count = SupplierGroup::where('name', trim($request->name))
                        ->count();
                if (!empty($count)) {
                    check(false, '物料分类已存在');
                }
                $countN = SupplierGroup::where('number', trim($request->number))
                        ->count();
                if (!empty($countN)) {
                ...
                break;
            case 'edited':
                $query = SupplierGroup::where('id', $request->id);
                $object = $query->first();
                if (empty(trim($request->number))) {
                ...
                $count = SupplierGroup::whereNot('id', $request->id)
                        ->where('name', trim($request->name))
                        ->count();
                if (!empty($count)) {
                    check(false, '物料分类名称已存在');
                }
                $countN = SupplierGroup::whereNot('id', $request->id)
