...
use App\Common\Models\InvoiceTypeGroup;
...
class InvoiceTypeGroupRepo extends Repository {

    protected $model;

    public function __construct() {
        $this->model = new InvoiceTypeGroup();
    ...
    public function edited($id, Request $request) {
        ...
        return InvoiceTypeGroup::where('id', $id)->update([
    ...
    public function add(Request $request) {
        $admin = Auth::guard('admin')->user();
        return InvoiceTypeGroup::insertGetId([
                    'status' => 'C',
                    'number' => !empty(trim($request->number)) ? trim($request->number) : $this->getInvoiceTypeGroupNo(),
                    'enable' => !empty($request->enable) ? intval($request->enable) : 1,
    ...
    public function enable(Request $request) {
        $admin = Auth::guard('admin')->user();
        $ids = $request->ids;
        return InvoiceTypeGroup::whereIn('id', $ids)->update([
                    'enable' => 1,
                    'modifier_id' => $admin->user_id,
                    'modify_time' => date('Y-m-d H:i:s'),
        ]);
    ...
    public function deleteData(Request $request) {
        $ids = $request->ids;
        return InvoiceTypeGroup::whereIn('id', $ids)->delete();
    }
     ...
     * @desc
     */
    public function setInvoiceTypeGroups(array &$list, string $field = 'user_type') {
        if (empty($list)) {
            return;
        }
        $userTypeIds = [];
        foreach ($list as &$val) {
            $val['invoice_type_group'] = '';
        ...
        foreach ($list as &$val) {
            if (isset($val[$field]) && isset($userTypeArr[$val[$field]])) {
                $val['invoice_type_group'] = $userTypeArr[$val[$field]];
            }
        }
    }
     ...
     * @desc 获取行业
     */
    public function setInvoiceTypeGroup(array &$arr, string $field = 'user_type') {
        if (empty($arr)) {
            return;
        }
        $userTypeId = '';
        ...
        $arr['invoice_type_group'] = '';
        if (empty($userTypeId)) {
            return $arr;
        }
        $arr['invoice_type_group'] = $this->model
                ->where('id', $userTypeId)
    ...
    public function export(Request $request) {
        ...
        $xlsName = "InvoiceTypeGroup_" . date("YmdHis", time()) . uniqid(); //文件名称
    ...
    public function importItemHandler($importData) {
        ...
        $newNumber = null;
        foreach ($data as $v) {
            $item['number'] = !empty(trim($v[0])) ? trim($v[0]) : $newNumber = $this->getInvoiceTypeGroupNo($newNumber); //执行标准
        ...
        return InvoiceTypeGroup::upsert($list, ['number'], ['name',
                    'category',
                    'modifier_id',
                    'modify_time',
                    'enable']);
     ...
     */
    public function getInvoiceTypeGroupNo(&$newNumber = null) {
        $prefix = 'ITG-';
