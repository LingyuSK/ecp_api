...
use App\Common\Models\InvoiceType;
...
class InvoiceTypeRepo extends Repository {

    protected $model;

    public function __construct() {
        $this->model = new InvoiceType();
    ...
    public function getList(Request $request, $filed = '*') {
        ...
        $list = $object->toArray();
        (new InvoiceTypeGroupRepo)->setInvoiceTypeGroups($list, 'group_id');
        return $list;
    }
    ...
    public function info($id) {
        ...
        $data = $object->toArray();
        (new InvoiceTypeGroupRepo)->setInvoiceTypeGroup($data, 'group_id');
        return $data;
    }
    ...
    public function edited($id, Request $request) {
        ...
        return InvoiceType::where('id', $id)->update([
                    'number' => trim($request->number),
                    'enable' => !empty($request->enable) ? intval($request->enable) : 1,
                    'status' => 'C',
                    'tax_control_code' => !empty($request->tax_control_code) ? trim($request->tax_control_code) : null,
                    'desc' => !empty($request->desc) ? trim($request->desc) : null,
                    'group_id' => !empty($request->group_id) ? intval($request->group_id) : null,
    ...
    public function add(Request $request) {
        $admin = Auth::guard('admin')->user();
        return InvoiceType::insertGetId([
                    'status' => 'C',
                    'number' => !empty(trim($request->number)) ? trim($request->number) : $this->getInvoiceTypeNo(),
                    'enable' => !empty($request->enable) ? intval($request->enable) : 1,
                    'tax_control_code' => !empty($request->tax_control_code) ? trim($request->tax_control_code) : null,
                    'desc' => !empty($request->desc) ? trim($request->desc) : null,
                    'group_id' => !empty($request->group_id) ? intval($request->group_id) : null,
    ...
    public function enable(Request $request) {
        $admin = Auth::guard('admin')->user();
        $ids = $request->ids;
        return InvoiceType::whereIn('id', $ids)->update([
                    'enable' => 1,
                    'modifier_id' => $admin->user_id,
                    'modify_time' => date('Y-m-d H:i:s'),
        ]);
    ...
    public function deleteData(Request $request) {
        $ids = $request->ids;
        return InvoiceType::whereIn('id', $ids)->delete();
    }
     ...
     * @desc
     */
    public function setInvoiceTypes(array &$list, string $field = 'invoice_type_id') {
        if (empty($list)) {
            return;
        }
        $userTypeIds = [];
        ...
        foreach ($list as &$val) {
            if (isset($val[$field]) && isset($userTypeArr[$val[$field]])) {
                $val['invoice_type'] = $userTypeArr[$val[$field]];
            }
        }
    }
     ...
     * @desc 获取行业
     */
    public function setInvoiceType(array &$arr, string $field = 'invoice_type_id') {
        if (empty($arr)) {
            return;
        }
        $userTypeId = '';
...
//          ->where('deleted_flag', 'N')
    ...
    public function export(Request $request) {
        ...
        $xlsName = "InvoiceType_" . date("YmdHis", time()) . uniqid(); //文件名称
    ...
    public function importItemHandler($importData) {
        ...
        $newNumber = null;
        foreach ($data as $v) {
            $item['number'] = !empty(trim($v[0])) ? trim($v[0]) : $newNumber = $this->getInvoiceTypeNo($newNumber); //执行标准
        ...
        return InvoiceType::upsert($list, ['number'], ['name',
                    'category',
                    'modifier_id',
                    'modify_time',
                    'enable']);
     ...
     */
    public function getInvoiceTypeNo(&$newNumber = null) {
        $prefix = 'IT-';
