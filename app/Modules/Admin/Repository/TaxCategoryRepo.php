...
use App\Common\Models\TaxCategory;
...
class TaxCategoryRepo extends Repository {
    ...
    public function getList(Request $request, $filed = '*') {
        ...
        $list = $object->toArray();
        (new TaxationSysRepo)->setTaxationSyss($list, 'taxation_sys_id');
        return $list;
    }
    ...
    public function info($id) {
        ...
        $data = $object->toArray();
        (new TaxationSysRepo)->setTaxationSys($data, 'taxation_sys_id');
        return $data;
    }
    ...
    public function edited($id, Request $request) {
        ...
        return TaxCategory::where('id', $id)->update([
                    'number' => trim($request->number),
                    'enable' => !empty($request->enable) ? intval($request->enable) : 1,
                    'status' => 'C',
                    'taxation_sys_id' => !empty($request->taxation_sys_id) ? intval($request->taxation_sys_id) : 0,
    ...
    public function add(Request $request) {
        $admin = Auth::guard('admin')->user();
        return TaxCategory::insertGetId([
                    'number' => !empty(trim($request->number)) ? trim($request->number) : $this->getTaxCategoryNo(),
                    'enable' => !empty($request->enable) ? intval($request->enable) : 1,
                    'status' => 'C',
                    'taxation_sys_id' => !empty($request->taxation_sys_id) ? intval($request->taxation_sys_id) : 0,
    ...
    public function enable(Request $request) {
        $admin = Auth::guard('admin')->user();
        $ids = $request->ids;
        return TaxCategory::whereIn('id', $ids)->update([
                    'enable' => 1,
                    'modifier_id' => $admin->user_id,
                    'modify_time' => date('Y-m-d H:i:s'),
        ]);
    ...
    public function deleteData(Request $request) {
        $ids = $request->ids;
        return TaxCategory::whereIn('id', $ids)->delete();
    }
     ...
     * @desc
     */
    public function setTaxCategorys(array &$list, string $field = 'tax_category_id') {
        if (empty($list)) {
            return;
        }
        $userTypeIds = [];
        ...
        foreach ($list as &$val) {
            if (isset($val[$field]) && isset($userTypeArr[$val[$field]])) {
                $val['tax_category'] = $userTypeArr[$val[$field]];
            }
        }
    }
     ...
     * @desc 获取行业
     */
    public function setTaxCategory(array &$arr, string $field = 'tax_category_id') {
        if (empty($arr)) {
            return;
        }
        $userTypeId = '';
    ...
    public function export(Request $request) {
        ...
        $xlsName = "TaxCategory_" . date("YmdHis", time()) . uniqid(); //文件名称
...
//数字转字母从65开始：
    ...
    public function importItemHandler($importData) {
        ...
        $newNumber = null;
        foreach ($data as $v) {
            $item['number'] = !empty(trim($v[0])) ? trim($v[0]) : $newNumber = $this->getTaxCategoryNo($newNumber); //执行标准
        ...
        return TaxCategory::upsert($list, ['number'], ['name',
                    'category',
                    'modifier_id',
                    'modify_time',
                    'enable']);
...
//加载文件并读取
     ...
     */
    public function getTaxCategoryNo(&$newNumber = null) {
        $prefix = 'TC-';
