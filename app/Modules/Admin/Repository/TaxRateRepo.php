...
class TaxRateRepo extends Repository {
    ...
    public function getList(Request $request, $filed = '*') {
        ...
        $object = $query->orderBy('tax_rate', 'DESC')->get();
        if (empty($object)) {
            return [];
        }
        $list = $object->toArray();
        (new TaxCategoryRepo)->setTaxCategorys($list, 'tax_category_id');
        return $list;
    }
    ...
    public function info($id) {
        ...
        $data = $object->toArray();
        (new TaxCategoryRepo)->setTaxCategory($data, 'tax_category_id');
        return $data;
    }
    ...
    public function edited($id, Request $request) {
        ...
        return TaxRate::where('id', $id)->update([
                    'number' => trim($request->number),
                    'enable' => !empty($request->enable) ? intval($request->enable) : 1,
                    'status' => 'C',
                    'tax_category_id' => !empty($request->tax_category_id) ? intval($request->tax_category_id) : null,
                    'tax_rate' => !empty($request->tax_rate) ? trim($request->tax_rate) : null,
                    'active_date' => !empty($request->active_date) ? trim($request->active_date) : null,
                    'exp_date' => !empty($request->exp_date) ? trim($request->exp_date) : null,
                    'country' => !empty($request->country) ? trim($request->country) : null,
    ...
    public function add(Request $request) {
        $admin = Auth::guard('admin')->user();
        return TaxRate::insertGetId([
                    'status' => 'C',
                    'number' => !empty(trim($request->number)) ? trim($request->number) : $this->getTaxRateNo(),
                    'enable' => !empty($request->enable) ? intval($request->enable) : 1,
                    'tax_category_id' => !empty($request->tax_category_id) ? intval($request->tax_category_id) : 0,
                    'tax_rate' => !empty($request->tax_rate) ? trim($request->tax_rate) : 0.00,
                    'active_date' => !empty($request->active_date) ? trim($request->active_date) : null,
                    'exp_date' => !empty($request->exp_date) ? trim($request->exp_date) : null,
                    'country' => !empty($request->country) ? trim($request->country) : 0,
    ...
    public function enable(Request $request) {
        $admin = Auth::guard('admin')->user();
        $ids = $request->ids;
        return TaxRate::whereIn('id', $ids)->update([
                    'enable' => 1,
                    'modifier_id' => $admin->user_id,
                    'modify_time' => date('Y-m-d H:i:s'),
        ]);
     ...
     * @desc
     */
    public function setTaxRates(array &$list, string $field = 'tar_rate_id', $fieldKey = 'tax_rate_name') {
        if (empty($list)) {
            return;
        }
        $userTypeIds = [];
        ...
        foreach ($list as &$val) {
            if (isset($val[$field]) && isset($userTypeArr[$val[$field]])) {
                $val[$fieldKey] = $userTypeArr[$val[$field]];
            }
        }
    }
     ...
     * @desc 获取行业
     */
    public function setTaxRate(array &$arr, string $field = 'tar_rate_id', $fieldKey = 'tax_rate_name') {
        if (empty($arr)) {
            return;
        }
        $userTypeId = '';
    ...
    public function export(Request $request) {
        ...
        $xlsName = "TaxRate_" . date("YmdHis", time()) . uniqid(); //文件名称
    ...
    public function importItemHandler($importData) {
        ...
        $newNumber = null;
        foreach ($data as $v) {
            $item['number'] = !empty(trim($v[0])) ? trim($v[0]) : $newNumber = $this->getTaxRateNo($newNumber); //执行标准
        ...
        return TaxRate::upsert($list, ['number'], ['name',
                    'category',
                    'modifier_id',
                    'modify_time',
                    'enable']);
     ...
     */
    public function getTaxRateNo(&$newNumber = null) {
        $prefix = 'TR-';
