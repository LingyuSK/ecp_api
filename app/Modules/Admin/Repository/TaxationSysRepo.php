...
use App\Common\Models\TaxationSys;
...
class TaxationSysRepo extends Repository {

    protected $model;

    public function __construct() {
        $this->model = new TaxationSys();
    ...
    public function getList(Request $request, $filed = '*') {
        ...
        $list = $object->toArray();
        (new CountryRepo)->setCountrys($list);
        return $list;
    }
    ...
    public function info($id) {
        ...
        $data = $object->toArray();
        (new CountryRepo)->setCountry($data);
        return $data;
    }
    ...
    public function edited($id, Request $request) {
        ...
        return TaxationSys::where('id', $id)->update([
                    ...
                    'country_id' => !empty($request->country_id) ? intval($request->country_id) : null,
    ...
    public function add(Request $request) {
        $admin = Auth::guard('admin')->user();
        return TaxationSys::insertGetId([
                    'status' => 'C',
                    'number' => !empty(trim($request->number)) ? trim($request->number) : $this->getTaxationSysNo(),
                    'enable' => !empty($request->enable) ? intval($request->enable) : 1,
                    'active_date' => !empty($request->active_date) ? trim($request->active_date) : null,
                    'exp_date' => !empty($request->exp_date) ? trim($request->exp_date) : null,
    ...
    public function enable(Request $request) {
        $admin = Auth::guard('admin')->user();
        $ids = $request->ids;
        return TaxationSys::whereIn('id', $ids)->update([
                    'enable' => 1,
                    'modifier_id' => $admin->user_id,
                    'modify_time' => date('Y-m-d H:i:s'),
        ]);
    ...
    public function deleteData(Request $request) {
        $ids = $request->ids;
        return TaxationSys::whereIn('id', $ids)->delete();
    }
     ...
     * @desc
     */
    public function setTaxationSyss(array &$list, string $field = 'user_type') {
        if (empty($list)) {
            return;
        }
        $userTypeIds = [];
        ...
        foreach ($list as &$val) {
            if (isset($val[$field]) && isset($userTypeArr[$val[$field]])) {
                $val['taxation_sys'] = $userTypeArr[$val[$field]];
            }
        }
    }
     ...
     * @desc 获取行业
     */
    public function setTaxationSys(array &$arr, string $field = 'user_type') {
        if (empty($arr)) {
            return;
        }
        $userTypeId = '';
    ...
    public function export(Request $request) {
        ...
        $xlsName = "TaxationSys_" . date("YmdHis", time()) . uniqid(); //文件名称
    ...
    public function importItemHandler($importData) {
        ...
        $newNumber = null;
        foreach ($data as $v) {
            $item['number'] = !empty(trim($v[0])) ? trim($v[0]) : $newNumber = $this->getTaxationSysNo($newNumber); //执行标准
        ...
        return TaxationSys::upsert($list, ['number'], ['name',
                    'category',
                    'modifier_id',
                    'modify_time',
                    'enable']);
     ...
     */
    public function getTaxationSysNo(&$newNumber = null) {
        $prefix = 'TS-';
