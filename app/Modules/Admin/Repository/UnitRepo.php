...
class UnitRepo extends Repository {
    ...
    public function edited($id, Request $request) {
        ...
        return Unit::where('id', $id)->update([
                    ...
                    'precision' => !empty($request->precision) ? intval($request->precision) : 0,
                    'status' => 'C',
    ...
    public function add(Request $request) {
        ...
        return Unit::insertGetId([
                    ...
                    'creator_id' => $admin->user_id,
                    'create_time' => date('Y-m-d H:i:s'),
                    'number' => !empty(trim($request->number)) ? trim($request->number) : $this->getUnitNo(),
                    'precision' => !empty($request->precision) ? intval($request->precision) : 0,
                    'enable' => !empty($request->enable) ? intval($request->enable) : 1,
                    'status' => 'C',
                    'creator_id' => $admin->user_id,
    ...
    public function enable(Request $request) {
        $admin = Auth::guard('admin')->user();
        $ids = $request->ids;
        return Unit::whereIn('id', $ids)->update([
                    'enable' => 1,
                    'modifier_id' => $admin->user_id,
                    'modify_time' => date('Y-m-d H:i:s'),
        ]);
     ...
     * @desc
     */
    public function setUnits(array &$list, string $field = 'unit_id') {
        if (empty($list)) {
            return;
        }
        $unitIds = [];
        ...
        $qurey->whereIn('id', $unitIds);
        $unitObjects = $qurey->get();
        if (empty($unitObjects)) {
            return $list;
        }

        $units = $unitObjects->toArray();

        $unitArr = [];
        foreach ($units as $unit) {
            $unitArr[$unit['id']] = $unit;
        }

        foreach ($list as &$val) {
            if (isset($val[$field]) && isset($unitArr[$val[$field]])) {
                $val[$field . '_name'] = trim($unitArr[$val[$field]]['name']);
                $val[$field . '_number'] = trim($unitArr[$val[$field]]['number']);
            }
        }
    }
     ...
     * @desc
     */
    public function setUnitIds(array &$list, string $field = 'unit', $fieldKey = 'unit_id') {
        if (empty($list)) {
            return;
        }
        $unitNames = [];
        ...
        $unitArr = [];
        foreach ($units as $unit) {
            $unitArr[$unit['name']] = $unit['id'];
        }

        foreach ($list as &$val) {
            if (isset($val[$field]) && isset($unitArr[$val[$field]])) {
                $val[$fieldKey] = trim($unitArr[$val[$field]]);
            }
        }
    }
     ...
     * @desc 获取行业
     */
    public function setUnit(array &$arr, string $field = 'unit_id') {
        if (empty($arr)) {
            return;
        }
        $groupId = '';
        ...
        $arr[$field . '_name'] = trim($group->name);
        $arr[$field . '_number'] = trim($group->number);
    }
    ...
    public function export(Request $request) {
        ...
        $xlsName = "Unit_" . date("YmdHis", time()) . uniqid(); //文件名称
    ...
    public function downloadExcel($name, $data = [], $head = []) {
        ...
        foreach ($data as $item) {
            ...
            $this->setExcelRow($sheet, 'C', $row, $item['precision'], 24);
    ...
    public function getPrecisionType($precisionTypeName) {
        switch ($precisionTypeName) {
            case '四舍五入':
                return '1';
            case '舍位':
                return '2';
    ...
    public function handelPrecisionType($precisionType) {
    ...
    public function importItemHandler($importData) {
        ...
        $newNumber = null;
        foreach ($data as $v) {
            $item['number'] = !empty(trim($v[0])) ? trim($v[0]) : $this->getUnitNo($newNumber); //执行标准
            $item['name'] = trim($v[1]); //执行标准
            $item['precision'] = trim($v[2]); //质保期
            ...
            $item['status'] = 'C'; //申报要素   
            $item['enable'] = trim($v[3]) === '可用' ? '1' : 0; //质保期 
            $list[] = $item;
        }
        return Unit::upsert($list, ['number'], ['name',
                    'precision',
                    'modifier_id',
                    'modify_time',
                    'enable']);
     ...
     */
    public function getUnitNo(&$newNumber = null) {
        $prefix = 'UM';
     ...
     * @desc
     */
    public function setUnitPrecisions(array &$list, string $field = 'unit_id') {
        if (empty($list)) {
            return;
        }
        $unitIds = [];
        ...
        $qurey = $this->model->select('id', 'precision');
        $qurey->whereIn('id', $unitIds);
        $unitObjects = $qurey->get();
        if (empty($unitObjects)) {
            return $list;
        ...
        $unitArr = [];
        foreach ($units as $unit) {
            $unitArr[$unit['id']] = $unit['precision'];
        }

        foreach ($list as &$val) {
            if (isset($val[$field]) && isset($unitArr[$val[$field]])) {
                $val['precision'] = trim($unitArr[$val[$field]]);
            }
        }
    }
