...
class InquiryRepo extends Repository {
    ...
    public function noticeinfo($id) {
        ...
        switch ($detail['biz_type']) {
            case '1':
                $ret['entry'] = (new InquiryEntryRepo)->getList($detail['src_bill_id']);
                $ret['inquiry'] = $this->info($detail['src_bill_id']);
                $ret['left_time'] = leftTimeDisplay($ret['inquiry']['end_date'] ?? 0);
                $ret['status'] = !empty($ret['inquiry']['biz_status']) ? $ret['inquiry']['biz_status'] : null;
                break;
            case 'A':
                $ret['entry'] = (new CompareEntryRepo)->getList($detail['src_bill_id']);
                $ret['inquiry'] = $this->info($detail['src_bill_id']);
                $ret['left_time'] = leftTimeDisplay($ret['inquiry']['end_date'] ?? 0);
                $ret['status'] = !empty($ret['inquiry']['biz_status']) ? $ret['inquiry']['biz_status'] : null;
                break;
